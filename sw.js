/* witsahead.org offline layer, version 202609150619. Pages and data: network first, cached copy when offline.
   Assets: cache first. Nothing is cached from other origins except the fonts. The visitor counter is never cached. */
var V='wa-202609150619', PAGES=["/", "/news/", "/projects/", "/fr/", "/fr/news/", "/news.json", "/fr/news.json", "/offline/", "/assets/favicon.svg", "/assets/app-icon-192.png"];
...
self.addEventListener('push',function(e){e.waitUntil(getPrefs().then(function(prefs){return fetch('https://notify.witsahead.org/latest',{cache:'no-store'}).then(function(r){return r.json();}).then(function(d){
 ...
 var ev=evs[0];if(!ev||ev.id===prefs.lastId)return;prefs.lastId=ev.id;var fr=prefs.lang==='fr';
 return setPrefs(prefs).then(function(){return self.registration.showNotification(fr&&ev.title_fr?ev.title_fr:ev.title,{body:fr&&ev.body_fr?ev.body_fr:(ev.body||''),icon:'/assets/app-icon-192.png',badge:'/assets/app-icon-192.png',tag:'wa-'+ev.topic,renotify:true,lang:fr?'fr':'en',data:{url:ev.url||'https://witsahead.org/'}});});
});}));});
